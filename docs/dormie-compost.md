...
- **Putt-distance prompt fires after EVERY hole regardless of putts** (observed on device). REGRESSION from original design intent: designed (Feb prototype) as a CONDITIONAL prompt tied to putts entered, one-putt case being the meaningful one (short-putt conversion; label switched '1ST PUTT DISTANCE' vs 'FIRST PUTT FROM'). Original rationale: putt-distance buckets turn raw putt counts into real putting stats (inside-5ft conversion, 5-15ft make rate). Current build lost the gating. Rework fix: restore putts-aware conditionality.
- **Double putt-distance capture on one-putts**: bucketed stat prompt (First Putt Distance) AND numeric Poleys prompt (Longest Putt) both fire for the same physical putt in incompatible formats (buckets vs exact ft). Collapse to ONE capture in the rework (already a rework-spec goal); flagged this session as trust-erosion.
...
- 2026-07-08 — BATCHED PHONE SESSION — verification debt (this run commits with a device-unverified label; verify all items in one phone session at end of run, then clear the list)
  ...
  3. **Match Play Stage 4a engine exposure (`f15320f`) — verified via 4b/4c on device.** No standalone surface; the derives are exercised by items 4-5. If items 4-5 pass, 4a is covered.

  4. **Match Play Stage 4b — 2v2 setup + live state (`a7f3eaf`) — two-sided.** Team side: Match Play round with 4 players → setup modal shows tap-to-move side columns (side never empties below 1), Best Ball/Aggregate pills, Gross/Net, team-aware perspective labels ("Side A (You/Kara)"); live banner tracks the chosen side mode (flip Best Ball↔Aggregate on a divergent board and watch the status change); dormie/match-closed moments fire on team match. Other side: 2-player Match Play setup + live banner identical to before (static columns, no side-mode pills); stroke/stableford/best-ball rounds unaffected (no match modal, no banner).

  5. **Match Play Stage 4c — team display (`c7e8ae0`) — two-sided.** Team: banner reads "You/Kara 2 UP thru 9" (one line, no truncation on device widths); post-round hero reads "You/Kara def. Drew/Tommy 3&2" with the clinch caption; enter scores past a clinch and confirm the post-round result stays frozen (team clinch drift). Other side: 1v1 match play banner + post-round hero character-identical to Stage 3b; Matchup competition tab unchanged.

- 2026-07-08 — MATCH PLAY STAGE 4 (2v2 TEAM) CODE-COMPLETE — device verification pending

  Stage 4 shipped in three sub-stage commits per docs/matchplay-architecture.md, all labeled device-unverified (batch plan): 4a `f15320f` team side-score variants (deriveBestBallSideScore best-ball-of-side, deriveAggregateSideScore strict-sum; net per player BEFORE best/sum; TMS.1-8 incl. singles-degeneration property), 4b `a7f3eaf` detection + setup + state (one catalog label — match_play's copy already promises 2v2; player count discriminates; matchSides first-two-vs-last-two default; matchSideMode best_ball|aggregate; team branch in matchPlayState memo), 4c `c7e8ae0` display (banner "You/Kara 2 UP thru 9"; resolveTeamMatchResult sharing the clinch-freeze walker — RMR.1-7 green across the refactor; team clinch-drift test TMR.2; post-round "You/Kara def. Drew/Tommy 3&2"). formatMatchState (Layer B) untouched throughout — the architecture's core promise held: teams were purely a Layer A + wiring problem. RyderCupHub / evaluateMatch / Matchup tab untouched (Phase 2). Unlocks the fourball/greensomes family: wiring their catalog labels onto matchSideMode='best_ball' + the existing derives is now a detection + copy exercise. Debt items 3-5 in the batched phone list. Tests 228 scoring + 39 poker, tsc 23.
...
- 2026-05-26 — MATCH PLAY — 1v1 COMPLETE END-TO-END (Stages 1–3b done; both open issues closed; updated 2026-06-09)

  **STATUS 2026-06-09: 1v1 MATCH PLAY IS COMPLETE END-TO-END (Stages 1–3b).** Setup → live banner → post-round result all work and are device-verified. Both open issues found on device are now CLOSED (see below). Remaining work is deferred and explicitly LAST: Stage 4 (team match) + Phase 2 (consolidation), against the proven engine.
  ...
  - Stage 3b — post-round match result + clinch-margin freeze, committed `ed922e8`. Renders the result as a hero in the always-visible Final Standings section ("You def. Kara 3&2" / "1 UP" / "Match halved", with CLOSED OUT ON HOLE N / TO THE LAST / ALL SQUARE sub-line). `matchSides` / `matchPerspective` / `matchScoreMode` threaded as optional props (fall back to first-two-players / URL scoreMode). **The clinch-margin freeze (the core fix):** new JSX-free helper `src/scoring/matchplay-result.ts` (`gir.ts` pattern for ts-node testability) — `resolveMatchResult` walks holes in play order and FREEZES the margin at the first hole where `lead > holesRemaining` (`remaining > 0` guard so a decisive final hole reads "1 UP", not "1&0"). The engine's `finalDisplay` drifts when scores are entered past the close-out (clinched 3&2 on 16, trailer wins 17–18 → engine emits "1 UP"); the helper is immune because it returns at the close-out hole. **Engine untouched** — live banner still uses `currentDisplay` correctly. 7 tests (RMR.1–7): the three played-past-clinch drift cases all freeze at 3&2; clinch-with-no-extra-holes; won-on-18 = "1 UP" not "1&0"; AS = HALVED; net-vs-gross divergence on the same board (proves strokes flow through the clinch recomputation).
  ...
  **TWO OPEN ISSUES found on device 2026-05-26 — BOTH CLOSED 2026-06-09:**

  1. **CLOSED — Post-round result rendering disconnect.** Stableford post-round table committed `436ce63` (verified on device — POS/PLAYER/POINTS standings render; strict `formatLabel === 'Stableford'` intentionally excludes Modified Stableford, which uses a different point scale). Match-play post-round result shipped in Stage 3b `ed922e8` (hero + clinch freeze, above). The original symptom (uncommitted branch absent from the build) was resolved by committing.

  2. **CLOSED — match-closed celebration firing on non-match rounds.** Fixed `7fc727e`: `checkDormieMoments` now takes an `isMatchPlay` param and the DORMIE/MATCH_CLOSED block is gated to it; early guards + skins-jackpot block stay unconditional (call-site gating would have killed the skins cinematic on non-match rounds). 6 tests (DMG.1–6) cover suppressed-when-`!isMatchPlay`, intact-when-`isMatchPlay`, and skins-unaffected. Verified on device both ways (Stableford blowout no longer fires; match-play dormie/close-out still celebrates).

  **TEST BASELINE (updated 2026-06-09): 209 scoring + 39 poker** (was 196 + 39). The +13 scoring: 6 DMG (celebration gate) + 7 RMR (clinch freeze). tsc baseline unchanged at 23.
